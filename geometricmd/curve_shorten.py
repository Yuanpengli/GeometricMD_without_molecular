...
def Q_I_metric(point,g_function):
    ...
    Args:
        ...
        g_function (func):
            this function is given by user in k_g_function_example directionary.
...
def compute_trajectory(trajectory, local_num_nodes, tol, configuration, length_function=length):
   ...
   # mass_matrix = np.diag(np.dstack((molecule.get_masses(),) * (dimension /
    ...
    if write_to_log:
        logging.basicConfig(format='%(asctime)s %(message)s', path='out/'+path+'.log', level=logging.INFO)
    else:
    ...
    if processes == 1:
        ...
        while True:
           ...
           # if i % save_frequency == 0:
            #    pickle.dump(trajectory, open('path'+'.pkl', "wb"))
            #    write_xyz_animation('path'+'.pkl','path'+'.xyz')
